...
class TeamRobot {
  ...
  static List<TeamRobot> listFromJson(List<dynamic> json) {
    return json == null ? List<TeamRobot>() : json.map((value) => TeamRobot.fromJson(value)).toList();
  }

  static Map<String, TeamRobot> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, TeamRobot>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = TeamRobot.fromJson(value));
    }
    return map;
  }

  // maps a json object with a list of TeamRobot-objects as value to a dart map
  static Map<String, List<TeamRobot>> mapListFromJson(Map<String, dynamic> json) {
